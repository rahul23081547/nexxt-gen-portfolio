  ...
  const toggleAudio = () => {
    if (audioRef.current) {
      if (isPlaying) {
        ...
        setShowVolumeSlider(false); // Hide volume slider when muted
      } else {
        audioRef.current.play().catch(error => {
          console.error("Audio playback failed:", error);
        });
      ...
      >
        ...
        <div className={`bg-dark-800/90 backdrop-blur-sm p-3 rounded-full shadow-lg border border-dark-600/50 transform transition-all duration-300 ease-in-out ${
          ...
          <input
            ...
            className="w-24 h-2 bg-dark-700 rounded-lg appearance-none cursor-pointer accent-primary-500"
            style={{
              background: `linear-gradient(to right, #ff0000 0%, #ff0000 ${volume * 100}%, #262626 ${volume * 100}%, #262626 100%)`
            }}
          />
        </div>
